...
class SquareGenerator implements Signal, Signal16 {
	...
	static {
		short temp;
		System.out.println("SquareGenerator(static): Reading square tables from file...");
		saw_table = new short[256 * 8];	// 8 octaves, 256 samples per table
		try {
			InputStream infile = new FileInputStream("square_table_256.dat");
		...
		*/
		...
		/*	for (int i = 0; i < 256; i++) {
			...
			System.out.println("ERROR: SquareGenerator(): cannot read square wavetable");
		}
	}

	public SquareGenerator(double p_srate) {
		phase = 0;
	...
	public short[] generateBlock16(double freq, int length) {
		...
		long index_hi, index_lo;	// for interpolating fractional values that fall between indices
		long high, low, frac, crossfade;
		int base_hi, base_lo;	// table consists of 8 waves, points to the bottom index of such a wave
		...
		for (i = 0; table_select > 0; i++)	// find the most significant bit for the next operation
